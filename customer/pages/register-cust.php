...
include 'dbcon.php';
// Assign customer signups to a valid branch when branching is enabled.
$branch_id = 0;
$members_has_branch = false;

$members_col_q = mysqli_query($con, "SHOW COLUMNS FROM members LIKE 'branch_id'");
if ($members_col_q && mysqli_num_rows($members_col_q) > 0) {
    $members_has_branch = true;
}

if ($members_has_branch) {
    $branch_q = mysqli_query($con, "SELECT id FROM branches ORDER BY id ASC LIMIT 1");
    if ($branch_q && ($branch_row = mysqli_fetch_assoc($branch_q))) {
        $branch_id = (int)$branch_row['id'];
    }
}
//code after connection is successfull
if ($members_has_branch) {
    $qry = "insert into members(fullname,username,password,dor,gender,services,amount,plan,address,contact,status,branch_id) values ('$fullname','$username','$password', CURRENT_TIMESTAMP,'$gender','$services','0','$plan','$address','$contact','Pending','$branch_id')";
} else {
