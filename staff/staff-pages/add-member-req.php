...
<body>
  ...
  <div id="content">
    <div id="content-header" style="padding: 20px;">
      <h1>Registration Status (Staff)</h1>
    </div>
            ...
            <div class="widget-content">
              ...
              if (isset($_POST['fullname'])) {
                ...
                if ($registrar_name === '') {
                  ...
                  $q = mysqli_query($conn, "SELECT fullname, username FROM staffs WHERE user_id='$id' LIMIT 1");
                ...
                mysqli_query($conn, "ALTER TABLE members ADD COLUMN IF NOT EXISTS id_doc_type VARCHAR(50) DEFAULT ''");
                mysqli_query($conn, "ALTER TABLE members ADD COLUMN IF NOT EXISTS id_document VARCHAR(255) DEFAULT ''");
                mysqli_query($conn, "ALTER TABLE members ADD COLUMN IF NOT EXISTS created_by VARCHAR(100) NULL");
                mysqli_query($conn, "ALTER TABLE members ADD COLUMN IF NOT EXISTS updated_by VARCHAR(100) NULL");
                mysqli_query($conn, "ALTER TABLE members ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP NULL DEFAULT NULL");
                ...
                mysqli_query($conn, "ALTER TABLE payment_history ADD COLUMN IF NOT EXISTS invoice_no VARCHAR(50) NULL");
                mysqli_query($conn, "ALTER TABLE payment_history ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP");
                ...
                if (!$result) {
                  ...
                  echo "</div>";
                  echo "<a class='btn btn-warning' href='edit-member.php'>Go Back</a>";
                } else {
                  $new_user_id = mysqli_insert_id($conn);
                  $fullname_esc = mysqli_real_escape_string($conn, $fullname);
                  $services_esc = mysqli_real_escape_string($conn, $services);
                  $discount_type_esc = mysqli_real_escape_string($conn, $discount_type);
                  $recorded_by_esc = mysqli_real_escape_string($conn, $registered_by);
                  ...
                  // Original staff file didn't include sms helper. But maybe it should?
                  // I'll skip SMS for now to minimize errors, or add it if requested. Admin had it.
