...
if (mysqli_num_rows($check_res) == 0) {
    ...
    if ($con->query($sql) === TRUE) {
        ...
        audit_log($con, 'staff', $actorId, 'attendance_check_in', 'attendance', $user_id . '|' . $date, 'Attendance checked in');
        echo json_encode([
            "status" => "success",
            "state" => "incomplete",
            "check_in" => $curr_time
    ...
    if (empty($row['check_out']) || strpos($row['check_out'], '0000') !== false) {
        ...
        if ($con->query($sql) === TRUE) {
            ...
            $actorId = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : '0';
            audit_log($con, 'staff', $actorId, 'attendance_check_out', 'attendance', $user_id . '|' . $date, 'Attendance checked out');
            echo json_encode([
                "status" => "success",
                "state" => "complete",
