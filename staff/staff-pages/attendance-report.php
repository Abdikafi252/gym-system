...
// Fetch members (Filtered by branch for staff)
...
if (isset($_SESSION['branch_id'])) {
    $members_qry .= " AND branch_id='{$_SESSION['branch_id']}'";
}
...
// Pre-fetch attendance data (No branch filter here, rely on JOIN and member list)
$attendance_data = [];
...
if (isset($_SESSION['branch_id'])) {
    $att_qry .= " AND m.branch_id='" . mysqli_real_escape_string($con, $_SESSION['branch_id']) . "'";
}
...
if (isset($_SESSION['branch_id'])) {
    $period_qry .= " AND branch_id='" . mysqli_real_escape_string($con, $_SESSION['branch_id']) . "'";
}
$period_qry .= " ORDER BY user_id ASC, paid_date DESC";
                                    ...
                                    <tr>
                                        ...
                                        ?>
                                            <td class="<?php echo $is_today ? 'today-col' : ''; ?>">
                                                <div class="icon-cell <?php echo ($is_future || $is_before_period_start || $is_after_period_end) ? 'future-day' : ''; ?>"
