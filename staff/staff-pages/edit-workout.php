    ...
    <div id="content">
        <div id="content-header">
            <div id="breadcrumb">
                ...
                <a href="#" class="current">Edit Workout</a>
            </div>
        </div>
        ...
        if (isset($_GET['id'])) {
            $id = $_GET['id'];
            $qry = "SELECT members.fullname, workout_plans.* 
                    FROM members 
                    JOIN workout_plans ON members.user_id = workout_plans.member_id 
                    WHERE members.user_id='$id'";
            $result = mysqli_query($con, $qry);
            $row = mysqli_fetch_array($result);

            // Extract DB JSON
            $db_plan_name = $row['plan_name'] ? $row['plan_name'] : 'Total Body Transformation';
            $db_plan_duration = $row['plan_duration'] ? $row['plan_duration'] : '4 Weeks';
        ...
        if (isset($_POST['update'])) {
            ...
            $qry = "UPDATE workout_plans SET plan_name='$u_plan_name', plan_duration='$u_plan_duration', plan_goal='$u_plan_goal', custom_data='$u_custom_data', assigned_by='$assigned_by', date_assigned='$date_assigned' WHERE id='$plan_id'";
            $result = mysqli_query($con, $qry);

            if ($result) {
                echo "<script>alert('Workout Plan Updated Successfully'); window.location='manage-workout.php';</script>";
            } else {
                echo "<script>alert('Error Updating Workout Plan');</script>";
            }
        }
        ...
        ?>
            ...
            <div class="container-fluid" x-data="workoutApp()">
                <form id="workoutForm" action="edit-workout.php?id=<?php echo $id; ?>" method="POST">
