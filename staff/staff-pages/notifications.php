...
$filter = $branch > 0 ? " AND branch_id='$branch'" : '';
$r = mysqli_query($con, "SELECT COUNT(*) c FROM members WHERE expiry_date = CURDATE() AND status='Active' $filter");
if ($r) $expiring_today = (int)mysqli_fetch_assoc($r)['c'];
$r = mysqli_query($con, "SELECT COUNT(*) c FROM members WHERE expiry_date < CURDATE() $filter");
if ($r) $expired_total = (int)mysqli_fetch_assoc($r)['c'];
$r = mysqli_query($con, "SELECT COUNT(*) c FROM members WHERE expiry_date <= DATE_ADD(CURDATE(), INTERVAL 7 DAY) $filter");
