...
<head>
    <title>Gym System Admin - Edit Diet Plan</title>
    <meta charset="UTF-8" />
    ...
    <div id="content">
        <div id="content-header">
            <div id="breadcrumb">
                ...
                <a href="#" class="current">Edit Diet Plan</a>
            </div>
        </div>
        ...
        if (isset($_GET['id'])) {
            $id = $_GET['id'];
            $qry = "SELECT members.fullname, diet_plans.* 
                    FROM members 
                    JOIN diet_plans ON members.user_id = diet_plans.member_id 
                    WHERE members.user_id='$id'";
            $result = mysqli_query($con, $qry);
            $row = mysqli_fetch_array($result);

            // Extract DB JSON
            $db_plan_name = $row['plan_name'] ? $row['plan_name'] : 'Muscle Gain';
            $db_plan_duration = $row['plan_duration'] ? $row['plan_duration'] : '7 Days';
            $db_plan_goal = $row['plan_goal'] ? $row['plan_goal'] : 'Muscle Building';

            $raw_json = $row['custom_data'];
        ...
        if (isset($_POST['update'])) {
            ...
            $u_plan_name = mysqli_real_escape_string($con, $_POST['plan_name']);
            $u_plan_duration = mysqli_real_escape_string($con, $_POST['plan_duration']);
            $u_plan_goal = mysqli_real_escape_string($con, $_POST['plan_goal']);
            $u_custom_data = mysqli_real_escape_string($con, $_POST['instruction']);

            $assigned_by = $_SESSION['user_id'];
            $date_assigned = date('Y-m-d');

            $qry = "UPDATE diet_plans SET plan_name='$u_plan_name', plan_duration='$u_plan_duration', plan_goal='$u_plan_goal', custom_data='$u_custom_data', assigned_by='$assigned_by', date_assigned='$date_assigned' WHERE id='$plan_id'";
            $result = mysqli_query($con, $qry);

            if ($result) {
                echo "<script>alert('Diet Plan Updated Successfully'); window.location='manage-diet.php';</script>";
            } else {
                echo "<script>alert('Error Updating Diet Plan');</script>";
            }
        }
        ...
        ?>
            ...
            <div class="container-fluid" x-data="dietApp()">
                <form id="dietForm" action="edit-diet.php?id=<?php echo $id; ?>" method="POST">
                    ...
                    <div class="diet-container">
                        ...
                        <div class="diet-header">
                            <div>
                                <div style="font-size:12px; color:#64748b; font-weight:700; text-transform:uppercase; margin-bottom:4px;">
                                    <i class="fas fa-user-circle"></i> Member: <?php echo $row['fullname']; ?>
                                </div>
                        ...
                        <div class="diet-footer">
                            ...
                            <button type="submit" name="update" class="save-btn"><i class="fas fa-sync-alt"></i> Update Diet Plan</button>
                        </div>
                    </div>
                </form>
            </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('dietApp', () => {
                ...
                // Get Existing JSON string from PHP
                let existingJsonStr = '<?php echo addslashes($raw_json); ?>';
                let parsedDays = [{
                        name: 'Sunday',
                        meals: []
                    },
                ...
                try {
                    let dbDays = JSON.parse(existingJsonStr);
                    if (dbDays && dbDays.length === 7 && dbDays[0].name) {
                        parsedDays = dbDays;
                    }
                } catch (e) {
                    console.log("Could not parse existing custom_data, using default.");
                }

                return {
                    planName: '<?php echo addslashes($db_plan_name); ?>',
                    planDuration: '<?php echo addslashes($db_plan_duration); ?>',
                    planGoal: '<?php echo addslashes($db_plan_goal); ?>',
                    activeDay: 0,
                    newMealName: '',
                    days: parsedDays,
