          ...
          <div class='widget-box'>
            ...
            <style>
              ...
              .staff-action-btn {
                ...
                color: #334155;
                transition: transform 0.25s cubic-bezier(.68,-.55,.27,1.55), box-shadow 0.25s, background 0.25s;
                position: relative;
                overflow: hidden;
              }

              .staff-action-btn:before {
                content: '';
                position: absolute;
                left: 0;
                top: 0;
            ...
            <div class="staff-grid">
              ...
              $qry = "SELECT * FROM staffs WHERE branch_id = '$branch_id'";
              ...
              ?>
                ...
                <div class="staff-card">
                  ...
                  <div class="staff-actions">
                    ...
                    <?php endif; ?>
                    <a class="staff-action-btn btn-delete" href="remove-staff.php?id=<?php echo (int)$row['user_id']; ?>" onclick="return confirm('Are you sure you want to delete this staff member?');">
                      <i class="fas fa-trash"></i> Delete
                    </a>
                  </div>
                            <!-- Staff Details Modal -->
                            <div id="staffDetailsModal" class="staff-details-modal" style="display:none;">
                              <div style="background:#fff; border-radius:12px; padding:24px; box-shadow:0 8px 32px rgba(15,23,42,0.12); position:relative;">
                                <button type="button" onclick="closeStaffDetailsModal()" style="position:absolute; top:18px; right:18px; background:#e2e8f0; color:#475569; border:none; border-radius:50%; width:36px; height:36px; font-size:18px; font-weight:700; cursor:pointer;">&times;</button>
                                <h2 style="font-size:22px; font-weight:800; color:#2563eb; margin-bottom:18px;">Staff Details</h2>
                            ...
                            <script>
                              function viewStaffDetails(source) {
                                ...
                                if (!data) return;
                                var html = '<div style="background:#fff; border-radius:18px; box-shadow:0 6px 32px rgba(30,41,59,0.16); border:1px solid #e2e8f0; padding:36px 28px; max-width:520px; margin:0 auto;">';
                                html += '<div style="display:flex; gap:28px; align-items:center; margin-bottom:24px;">';
                                html += '<div style="width:120px; height:120px; border-radius:50%; background:#e0f2fe; display:flex; align-items:center; justify-content:center; font-size:38px; color:#0284c7; flex-shrink:0; box-shadow:0 2px 8px rgba(0,0,0,0.07);">';
                                if (data.photo) {
                                  html += '<img src="' + data.photo + '" alt="Staff Photo" style="width:120px; height:120px; border-radius:50%; object-fit:cover; border:3px solid #e2e8f0;">';
                                } else {
                                  html += '<i class="fas fa-user-tie"></i>';
                                }
                                html += '</div>';
                                html += '<div style="flex:1; min-width:180px;">';
                                html += '<div style="font-size:24px; font-weight:800; color:#0f172a; margin-bottom:8px;">' + (data.fullname || 'N/A') + '</div>';
                                html += '<div style="font-size:16px; color:#2563eb; margin-bottom:8px;">@' + (data.username || '') + '</div>';
                                html += '<div style="font-size:15px; color:#fff; background:#2563eb; display:inline-block; padding:4px 16px; border-radius:8px; font-weight:700; margin-bottom:8px;">' + (data.designation || 'N/A') + '</div>';
                                html += '<div style="font-size:15px; color:#64748b; margin-bottom:4px;">Gender: <span style="color:#334155; font-size:16px;">' + (data.gender || 'N/A') + '</span></div>';
                                html += '</div>';
                                html += '</div>';
                                html += '<hr style="border:none; border-top:1px solid #e2e8f0; margin:18px 0;">';
                                html += '<div style="display:flex; flex-wrap:wrap; gap:18px;">';
                                html += '<div style="flex:1; min-width:180px; font-size:15px; color:#64748b;">Phone:<br><span style="color:#334155; font-size:16px; font-weight:600;">' + (data.contact || 'N/A') + '</span></div>';
                                html += '<div style="flex:1; min-width:180px; font-size:15px; color:#64748b;">Email:<br><span style="color:#334155; font-size:16px; font-weight:600;">' + (data.email || 'N/A') + '</span></div>';
                                html += '<div style="flex:1; min-width:180px; font-size:15px; color:#64748b;">Address:<br><span style="color:#334155; font-size:16px; font-weight:600;">' + (data.address || 'N/A') + '</span></div>';
                                html += '</div>';
                                html += '</div>';
    ...
    <div class="modal-body" style="padding: 25px;">
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
        ...
        <div style="display: flex; flex-direction: column; grid-column: span 2;">
          <span style="font-size: 11px; color: #94a3b8; font-weight: 700; text-transform: uppercase;">Phone Number</span>
          <span id="stf_contact" style="font-size: 15px; color: #1e293b; font-weight: 600;"></span>
        </div>
        <div style="display: flex; flex-direction: column; grid-column: span 2;">
          <span style="font-size: 11px; color: #94a3b8; font-weight: 700; text-transform: uppercase;">Email Address</span>
  ...
  <script type="text/javascript">
    ...
    function viewStaffDetails(source) {
      try {
        ...
        alert("Sorry, staff details cannot be displayed at the moment.");
      }
    }
  </script>
</body>
